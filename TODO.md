...
- [x] ✅ **Bus Operator Full Portal** — Bus operators can add/manage buses with OneTime/Daily/Weekly recurring schedules, amenity selection, driver/staff details, photo URL, seat layout config. Interactive seat map (like flights) with date selector → bus selection → visual grid; bulk booking, live booking status, and cancellation with 90% wallet refund. New `Bus` entity + EF migration. 11 new API endpoints at `/api/v1/bus-operator/*`.
- [x] ✅ **Bus Operator UX Enhancements** — City autocomplete dropdowns, conditional date picker (OneTime only), overnight journey support (+1 day arrival offset), auto seat row calculation, per-berth pricing (upper/lower for sleeper buses), upper/lower berth visual in seat map, return journey creation with same-config/driver/number toggles.
...
- [x] ✅ **Hotel booking management flow** — Full hotel ops: check-in (room assignment), additional charges (food/laundry/room service/minibar/etc.), check-out with payment method selection, invoice PDF generation, checkout email with invoice + rating link. `HotelBookingCharge` entity + migration. 8 new API endpoints. Complete `HotelBookingsPage` rewrite with 6 modals, search (name/email/phone/ref), status tabs, checkout-today indicators.
- [x] ✅ **Hotel profile UX** — City field disabled with lock icon, amenities as 20-option multi-select pill buttons, gallery images via upload or URL, main image via upload or URL.
