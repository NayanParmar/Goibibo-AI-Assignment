...
### Bugs
- [ ] 🔴 **Coupon discount not applied to `FinalAmount`** — `BookAsync` sets `DiscountAmount: 0` regardless of `CouponCode`. Coupon lookup and discount calculation need to be wired in `FlightService.BookAsync` and `HotelService.BookAsync`.
...
- [ ] 🟢 **DataSeeder always drops flight-linked bookings on restart** — seed bookings for `john@example.com` get deleted on every restart because `SeedFlightsAsync` always refreshes flights. Consider seeding bookings against a stable ReferenceId.
